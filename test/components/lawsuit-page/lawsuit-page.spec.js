...
describe('LawsuitPage component', function () {
  ...
  describe('component mounted', function () {
    it('should not call fetchLawsuit if the lawsuit is not null', function () {
      const fetchLawsuitSpy = spy();
      mount(
        <Provider store={ store }>
          <MemoryRouter>
            <HelmetProvider>
              <LawsuitPage
                fetchLawsuit={ fetchLawsuitSpy }
                lawsuitCaseNo={ '123' }
                lawsuit={ null }
              />
            </HelmetProvider>
      ...
      fetchLawsuitSpy.calledWith('123').should.be.true();
    });

    it('should call fetchLawsuit if the lawsuit is null', function () {
      const fetchLawsuitSpy = spy();
      mount(
        <Provider store={ store }>
          <MemoryRouter>
      ...
      fetchLawsuitSpy.calledWith('123').should.be.false();
    });
  });
