    ...
    context('undo card', function () {
      context('adding relevant coaccusal to pinboard', function () {
        const updatedRelevantDocumentsResponse = getPaginationResponse(
          baseRelevantDocumentsUrl,
          (number) => _.times(number).map(generateRelevantDocument),
          6, 0, 6
        );
        ...
        const updatedRelevantComplaintsResponse = getPaginationResponse(
          baseRelevantComplaintsUrl,
          (number) => _.times(number, index => generateRelevantComplaint(`${index}`)),
          8, 0, 8
        );
        ...
        beforeEach(function (client, done) {
          ...
          api.mock('GET', baseRelevantDocumentsUrl, 200, updatedRelevantDocumentsResponse);

          api.mock('GET', baseRelevantCoaccusalsUrl, 200, updatedRelevantCoaccusalsResponse);

          api.mock('GET', baseRelevantComplaintsUrl, 200, updatedRelevantComplaintsResponse);
