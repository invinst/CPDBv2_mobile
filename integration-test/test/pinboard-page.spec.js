...
const {
  ...
  officersSummary,
  complainantsSummary,
} = require(__dirname + '/../mock-data/widgets');
...
describe('Pinboard Page', function () {
  beforeEach(function (client, done) {
    ...
    api
      ...
      .replyOnce(200, mockData.firstRelevantDocumentsResponse);
    api
      .onGet(`${mockData.baseRelevantDocumentsUrl}limit=4&offset=4`)
      .replyOnce(200, mockData.secondRelevantDocumentsResponse);
    api
      .onGet(`${mockData.baseRelevantDocumentsUrl}limit=4&offset=8`)
      .replyOnce(200, mockData.lastRelevantDocumentsResponse);

    api
      .onGet(mockData.baseRelevantCoaccusalsUrl)
      .replyOnce(200, mockData.firstRelevantCoaccusalsResponse);
    api
      .onGet(`${mockData.baseRelevantCoaccusalsUrl}limit=4&offset=4`)
      .replyOnce(200, mockData.secondRelevantCoaccusalsResponse);
    api
      .onGet(`${mockData.baseRelevantCoaccusalsUrl}limit=4&offset=8`)
      .replyOnce(200, mockData.lastRelevantCoaccusalsResponse);

    api
      .onGet(mockData.baseRelevantComplaintsUrl)
      .replyOnce(200, mockData.firstRelevantComplaintsResponse);
    api
      .onGet(`${mockData.baseRelevantComplaintsUrl}limit=4&offset=4`)
      .replyOnce(200, mockData.secondRelevantComplaintsResponse);
    api
      .onGet(`${mockData.baseRelevantComplaintsUrl}limit=4&offset=8`)
      .replyOnce(200, mockData.lastRelevantComplaintsResponse);

    api
      .onPut('/api/v2/mobile/pinboards/5cd06f2b/', mockData.updatePinboardDescriptionParams)
      .reply(200, mockData.updatedPinboardDescription);
        ...
        beforeEach(function (client, done) {
          api
            .onPut(
              ...
              { 'officer_ids': [], crids: ['1234567'], 'trr_ids': ['1234'], title: 'Pinboard Title' }
            )
            .reply(
              200,
              {
      ...
      context('pinned complaints', function () {
        beforeEach(function (client, done) {
          api
            .onPut(
              '/api/v2/mobile/pinboards/5cd06f2b/',
              { 'officer_ids': [1234], crids: [], 'trr_ids': ['1234'], title: 'Pinboard Title' }
            )
            .reply(
              200,
              {
  ...
  context('pinboard section', function () {
    ...
    it('should display spinner after update title', function (client) {
      api
        ...
        .delay(3000)
        .reply(200, mockData.updatedPinboardTitle);

      api.onGet('/api/v2/mobile/pinboards/').reply(200, mockData.pinboardsList.list);
    ...
    context('undo card', function () {
      context('adding relevant coaccusal to pinboard', function () {
        ...
        beforeEach(function (client, done) {
          ...
          api.onGet(mockData.baseRelevantDocumentsUrl).replyOnce(200, updatedRelevantDocumentsResponse);

          api.onGet(mockData.baseRelevantCoaccusalsUrl).replyOnce(200, updatedRelevantCoaccusalsResponse);

          api.onGet(mockData.baseRelevantComplaintsUrl).replyOnce(200, updatedRelevantComplaintsResponse);
  ...
  context('manage pinboards', function () {
    beforeEach(function (client, done) {
      api.onGet('/api/v2/mobile/pinboards/').reply(200, mockData.pinboardsList.list);
      api.onGet('/api/v2/mobile/pinboards/23ffd689/').reply(200, mockData.pinboardsList.pinboards[0]);
      api.onGet('/api/v2/mobile/pinboards/7e1e3c88/').reply(200, mockData.pinboardsList.pinboards[1]);
      done();
    });
    ...
    context('clicking on pinboard item', function () {
      ...
      context('users remove pinned item from current pinboard and current pinboard is saved', function () {
        it('should go to target pinboard detail page', function (client) {
          api
            .onPut('/api/v2/mobile/pinboards/5cd06f2b/', mockData.updatePinboardCrsRequest)
            .reply(200, mockData.updatePinboardCrsResponse);
          const pinboardsListSection = this.pinboardPage.section.pinboardsListSection;
          const secondPinboardItem = pinboardsListSection.section.secondPinboardItem;
      ...
      context('users add relevant item to current pinboard and current pinboard is saved', function () {
        it('should go to target pinboard detail page', function (client) {
          api
            .onPut('/api/v2/mobile/pinboards/5cd06f2b/', mockData.updatePinboardCoaccusalsRequest)
            .reply(200, mockData.updatePinboardCoaccusalsResponse);
          const pinboardsListSection = this.pinboardPage.section.pinboardsListSection;
          const secondPinboardItem = pinboardsListSection.section.secondPinboardItem;
    ...
    context('clicking on create new pinboard button in menu', function () {
      it('should create an empty pinboard', function (client) {
        api.onPost('/api/v2/mobile/pinboards/').reply(201, mockData.pinboardsList.pinboards[3]);
        const pinboardsListSection = this.pinboardPage.section.pinboardsListSection;
        this.pinboardPage.click('@pinboardsListButton');
      ...
      it('should display spinner on creating new pinboard', function (client) {
        api.onPost('/api/v2/mobile/pinboards/').delay(3000).reply(201, mockData.pinboardsList.pinboards[3]);
        const pinboardsListSection = this.pinboardPage.section.pinboardsListSection;
        const firstPinboardItem = pinboardsListSection.section.firstPinboardItem;
        this.pinboardPage.click('@pinboardsListButton');
      ...
      it('should create an empty pinboard if pinboard is saving and user confirm yes', function (client) {
        api
          .onPost('/api/v2/mobile/pinboards/')
          .reply(201, mockData.pinboardsList.pinboards[3]);
        api
          .onPut('/api/v2/mobile/pinboards/5cd06f2b/', mockData.updatePinboardCrsRequest)
          .delay(10000)
          .reply(200, mockData.updatePinboardCrsResponse);
    ...
    describe('click on pinboard item actions button', function () {
      ...
      context('clicking on Duplicate this pinboard button', function () {
        it('should duplicate pinboard', function (client) {
          api
            .onPost('/api/v2/mobile/pinboards/', mockData.pinboardsList.duplicatePinboardRequest)
            .reply(200, mockData.pinboardsList.duplicatePinboardResponse);
          const pinboardsListSection = this.pinboardPage.section.pinboardsListSection;
          const firstPinboardItem = pinboardsListSection.section.firstPinboardItem;
          this.pinboardPage.click('@pinboardsListButton');
      ...
      context('clicking on remove pinboard button', function () {
        context('remove not current pinboard', function () {
          it('should remove item from pinboards list', function (client) {
            api.onDelete('/api/v2/mobile/pinboards/7e1e3c88/').reply(200);
            const pinboardsListSection = this.pinboardPage.section.pinboardsListSection;
            const firstPinboardItem = pinboardsListSection.section.firstPinboardItem;
            const secondPinboardItem = pinboardsListSection.section.secondPinboardItem;
            const thirdPinboardItem = pinboardsListSection.section.thirdPinboardItem;
        ...
        context('remove last pinboard', function () {
          it('should create new pinboard', function (client) {
            api.onDelete('/api/v2/mobile/pinboards/5cd06f2b/').reply(200);
            api.onDelete('/api/v2/mobile/pinboards/7e1e3c88/').reply(200);
            api.onDelete('/api/v2/mobile/pinboards/3a160339/').reply(200);

            api
              .onGet('/api/v2/mobile/pinboards/latest-retrieved-pinboard/?create=true')
              .reply(200, mockData.pinboardsList.createdPinboard);
  ...
  context('Summary widgets', function () {
    context('Complaint Summary section', function () {
      it('should render complaint summary section', function (client) {
        api
          .onGet('/api/v2/mobile/pinboards/5cd06f2b/complaint-summary/')
          .delay(3000)
          .reply(200, complaintSummary);
        client.resizeWindow(500, 1000);
    ...
    context('TRR Summary section', function () {
      it('should render TRR summary section', function (client) {
        api
          .onGet('/api/v2/mobile/pinboards/5cd06f2b/trr-summary/')
          .delay(3000)
          .reply(200, trrSummary);
        client.resizeWindow(500, 1000);
    ...
    context('Officers Summary section', function () {
      it('should render Officers summary section', function (client) {
        api
          .onGet('/api/v2/mobile/pinboards/5cd06f2b/officers-summary/')
          .delay(3000)
          .reply(200, officersSummary);
        client.resizeWindow(500, 1000);
    ...
    context('Complainants Summary section', function () {
      it('should render Complainants summary section', function (client) {
        api
          .onGet('/api/v2/mobile/pinboards/5cd06f2b/complainants-summary/')
          .delay(3000)
          .reply(200, complainantsSummary);
        client.resizeWindow(500, 1000);
        const complainantsSummaryWidget = this.pinboardPage.section.complainantsSummaryWidget;

        client.assertInViewport(complainantsSummaryWidget.selector, complainantsSummaryWidget.locateStrategy, false);

        this.pinboardPage.waitForElementVisible('@navigationNextButton');
        this.pinboardPage.click('@navigationNextButton');
        this.pinboardPage.waitForElementVisible('@navigationNextButton');
        this.pinboardPage.click('@navigationNextButton');
        client.waitForAnimationEnd(complainantsSummaryWidget.selector, complainantsSummaryWidget.locateStrategy);
        client.assertInViewport(complainantsSummaryWidget.selector, complainantsSummaryWidget.locateStrategy);

        complainantsSummaryWidget.expect.element('@spinner').to.be.visible;
        complainantsSummaryWidget.expect.element('@spinner').to.not.be.present.before(5000);

        complainantsSummaryWidget.expect.element('@widgetTitle').text.to.equal('COMPLAINANTS');

        const raceSection = complainantsSummaryWidget.section.raceSection;
        const genderSection = complainantsSummaryWidget.section.genderSection;
        const raceCharts = raceSection.elements.charts;
        const genderCharts = genderSection.elements.charts;
